...
namespace Mb.Infrastructure.Services.Handlers;
...
public class HttpClientHandler: IHttpClientHandler
{
    ...
    public async Task<ServiceResult<TResult>> GenericRequest<TRequest, TResult>(string clientApi, string url, 
        CancellationToken cancellationToken, MethodType method = MethodType.Get, TRequest requestEntity = null) 
        where TRequest : class where TResult : class
    {
        ...
        try
        {
            _logger.LogInformation("HttpClient Request: {RequestName} {@Request}", requestName, 
                requestEntity);
            ...
            if (response != null && response.IsSuccessStatusCode)
            {
                ...
                return ServiceResult.Success<TResult>(data);
            }
